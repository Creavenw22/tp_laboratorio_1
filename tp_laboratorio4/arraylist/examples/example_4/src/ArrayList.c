...
{
    int returnAux = -1;
    if(index>=0 && pList!=NULL && pElement!=NULL)
    {
        if(index<=pList->size)
        {
...
ArrayList* al_clone(ArrayList* pList)
{
    ...
    if(pList!=NULL)
    {
        ...
        if((returnAux=al_newArrayList())!=NULL)
        {
            for(i=0; i<pList->size; i++)
            {
                pList->add(returnAux,pList->pElements[i]);
            }
        }
    }
...
ArrayList* al_subList(ArrayList* pList,int from,int to)
{
    ArrayList* returnAux = NULL;
    int i;
    if(pList!=NULL && pList->size>0 && from>=0 && from<=to)
    {
        if(to<=pList->size)
        {
            if((returnAux=al_newArrayList())!=NULL)
        ...
        if(pList->size>1)
        {
            for(i=0; i<pList->size-1; i++)
            {
                for(j=i+1; j<pList->size; j++)
                {
                    ...
                    {
                        if(order==0 && pFunc(pList->pElements[i],pList->pElements[j])==-1)
                        {
                            auxElement=pList->pElements[i];
                            pList->pElements[i]=pList->pElements[j];
                            pList->pElements[j]=auxElement;
...
int expand(ArrayList* pList,int index)
{
    ...
    if(pList!=NULL)
    {
        ...
        for(i=0; i<pList->size; i++)
        {
            pList->pElements[pList->size-i]=pList->pElements[pList->size-1-i];
            if(pList->pElements[index]==pList->pElements[index+1])
            {
                break;
            }
        }
